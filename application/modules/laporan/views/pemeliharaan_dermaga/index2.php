				...
				<div class="row">
					<div class="col-md-12">
						<div class="table-toolbar" style="margin-bottom: 0px">
							<div class="row">
								...
								<div class="col-md-3" style="padding-left: 0px;">
									<div class="form-group">
										<div class="input-group">
											<span class="input-group-addon">Dermaga</span>
						...
						<div id="master_tabel" style="display: none">
							...
							<?= generate_button('laporan/pemeliharaan_dermaga', 'download_pdf', '<button id="printerkita" class="btn btn-sm btn-default"><i class="fa fa-file-pdf-o" style="color: #ea5460"></i> PDF</button>') ?>
							<?= generate_button('laporan/pemeliharaan_dermaga', 'download_excel', '<button id="excelkita" class="btn btn-sm btn-default"><i class="fa fa-file-excel-o" style="color: #ea5460"></i> EXCEL</button>') ?>
							...
							<table class="tabel full-width" align="center">
								<tr>
									...
									<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
										FORMULIR LAPORAN PENDAPATAN<br>PEMELIHARAAN DERMAGA PELABUHAN PER-SHIFT</span>
									</td>
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {
			...
			$("#port").change(function() {
				$.ajax({
						method: "GET",
						url: "pemeliharaan_dermaga/get_dock/" + $("#port").val(),
						type: "html"
					})
					.done(function(msg) {
						$("#dock").html(msg);
			...
			$("#printerkita").on("click", function(e) {
				...
				dock_name = $('#dock').find(":selected").text();

				window.location = "<?php echo site_url('laporan/pemeliharaan_dermaga/get_pdf?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&shift=" + shift +
			...
			$("#excelkita").on("click", function(e) {
				...
				window.location = "<?php echo site_url('laporan/pemeliharaan_dermaga/get_excel?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&shift=" + shift +
					"&ship_class=" + ship_class +
			...
			$("#cari").on("click", function(e) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('laporan/pemeliharaan_dermaga/detail') ?>",
					dataType: 'json',
					data: {
						port: $("#port").val(),
						datefrom: $("#datefrom").val(),
					...
					success: function(json) {
						...
						if (json.code == 200) {
							...
							html += "<td class='text-center'> TARIF (Dmg) </td>";
							...
							$.each(json.penumpang, function(i, item) {
								...
								total_pendapatan_penumpang += Number(item.dock_fee);
								...
								if (item.dock_fee != null) {
									pendapatanku = formatIDR(item.dock_fee)
								}
							...
							$.each(json.kendaraan, function(i, item) {
								...
								total_pendapatan_kendaraan += Number(item.dock_fee);
							...
							var new_shiftku = ": " + $('#shift').find(":selected").text();
