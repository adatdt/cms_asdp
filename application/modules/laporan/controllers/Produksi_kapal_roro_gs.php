<?php
error_reporting(0);
class Produksi_kapal_roro_gs extends MY_Controller {

	public function __construct(){
		parent::__construct();
		$this->load->model('produksi_kapal_roro_gs_model', 'produksi_roro');
	...
	public function index(){
		...
		$data = array(
			...
			'content' 		=> 'produksi_kapal_roro_gs/index',
	...
	public function detail(){
		...
		$all_data = $this->produksi_roro->get_all_data($datefrom, $dateto, $port, $shift,$ticketType);
        ...
        if ($header) {
			...
			// $tipeTiket = $header->ticketTypeku; // belum naik tiket manual
		...
		$output =  array(
			...
			'regu' => $regu,
			'jam' => $jam_shift,
			// 'tipeTiket'=>$ticketTypeku,  // belum naik tiket manual
		);
    ...
    function get_pdf(){
        ...
        if ($header) {
            ...
            $shift = $header->shift_name;
            $regu = $header->team_name;
			$ticketTypeku = $header->ticketTypeku;
		}
		...
		$output =  array(
			...
			'regu' => $regu,
			'jam' => $jam_shift,
			'tipeTiket'=>empty($ticketTypeku)?"SEMUA":$ticketTypeku,		
		);
	...
	function get_excel()
	{
        ...
        $shift = $this->input->get('shiftku');
		$regu = "Semua";
		$tipeTiket= $this->input->get('ticketTypeku');
		...
		$sub_head = array('','','','','','','','','','DEWASA','LANSIA','ANAK','BAYI','','I','II','III','IVA','IVB','VA','VB','VIA','VIB','VII','VIII','IX','','');
		...
		// $writer->writeSheetRow($sheet1, array("","REGU",$regu,"","JAM",$jam_shift,"","TIPE TIKET",empty($tipeTiket)?"SEMUA":$tipeTiket)); // belum deploy tiket manual
		...
		$countAllData=count((array)$all_data);
		$getLowFooter=$countAllData+7;
		...
		$writer->markMergedCell($sheet1, $start_row=5, $start_col=9, $end_row=5, $end_col=12);
		$writer->markMergedCell($sheet1, $start_row=5, $start_col=13, $end_row=6, $end_col=13);
		$writer->markMergedCell($sheet1, $start_row=5, $start_col=14, $end_row=5, $end_col=25);
		$writer->markMergedCell($sheet1, $start_row=5, $start_col=26, $end_row=6, $end_col=26);
		$writer->markMergedCell($sheet1, $start_row=5, $start_col=27, $end_row=6, $end_col=27);


		//tanpa lansia
		// $writer->markMergedCell($sheet1, $start_row=5, $start_col=9, $end_row=5, $end_col=11);
		// $writer->markMergedCell($sheet1, $start_row=5, $start_col=12, $end_row=6, $end_col=12);
		// $writer->markMergedCell($sheet1, $start_row=5, $start_col=13, $end_row=5, $end_col=24);
		...
		$writer->markMergedCell($sheet1, $start_row=$getLowFooter, $start_col=0, $end_row=$getLowFooter, $end_col=8);
