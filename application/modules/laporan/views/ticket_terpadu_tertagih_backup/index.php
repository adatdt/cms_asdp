									...
									<div class="col-md-3" style="padding-left: 30px;padding-right: 8px;">
										<div class="form-group">
											<div class="input-group">
												...
												<select id="merchant" class="form-control select2" dir="">
													<?php echo $merchant ?>
												</select>
											</div>
										</div>
				...
				<div id="master_tabel" style="display: none">
					...
					<table class="tabel full-width" align="center">
						<tr>
							...
							<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
								LAPORAN PRODUKSI DAN PENDAPATAN<br /> TIKET TERPADU TERTAGIH PER-SHIFT</span>
							</td>
					...
					<table class="table table-333 table-bordered full-width" align="center">
						...
						<tr>
							...
							<td style="border-right: none !important;">MERCHANT</td>
							<td style="border-left: none !important;"><span id="merchantku"></span></td>
						</tr>
					</table>
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		// 	$.ajax({
		// 		method: "GET",
		// 		url: "ticket_terpadu_tertagih/get_regu/"+$("#port").val(),
		// 		type: "html"
		// 	})
		// 	.done(function( msg ) {
		// 		$("#regu").html(msg);
		...
		$("#printerkita").on("click", function(e) {
			...
			if (port != null) {
				window.location = "<?php echo site_url('laporan/ticket_terpadu_tertagih/get_pdf?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&petugas=" + petugas +
					"&vm=" + vm +
		...
		$("#excelkita").on("click", function(e) {
			...
			if (port != null) {
				window.location = "<?php echo site_url('laporan/ticket_terpadu_tertagih/get_excel?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&regu=" + regu +
					"&petugas=" + petugas +
		...
		$("#cari").on("click", function(e) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('laporan/ticket_terpadu_tertagih') ?>",
				dataType: 'json',
				data: {
					port: $("#port").val(),
					datefrom: $("#datefrom").val(),
				...
				success: function(json) {
					...
					if (json.code == 200) {
						...
						html += "</tr>";
						angka = 1;
						total_produksi_ifcs_reedem = 0;
						total_pendapatan_ifcs_reedem = 0;

						$.each(json.ifcs_reedem, function(i, item) {
							total_produksi_ifcs_reedem += Number(item.produksi);
							total_pendapatan_ifcs_reedem += Number(item.pendapatan);
						...
						html += "<tr><td colspan='3'> <b>Sub Total</b></td><td class='text-right'>" + formatIDR(total_produksi_ifcs_reedem) + "</td><td class='text-right'>" + formatIDR(total_pendapatan_ifcs_reedem) + "</td></tr>";
						...
						jumlah_produksi = Number(total_produksi_cash + total_produksi_cashless + total_produksi_online + total_produksi_ifcs + total_produksi_ifcs_reedem + total_produksi_b2b);
						jumlah_pendapatan = Number(total_pendapatan_cash + total_pendapatan_cashless + total_pendapatan_online + total_pendapatan_ifcs + total_pendapatan_ifcs_reedem + total_pendapatan_b2b);
						...
						var merchantku = ": " + $('#merchant').find(":selected").text();
						...
						var reguku = ": " + json.regu;
						var nama_lintasan = json.lintasan;
						var status_approve = json.status_approve.toUpperCase();
						...
						$("#vmkita").html(vmku);
						$("#merchantku").html(merchantku);

						$("#kelaskuu").html(kelaskuu.toUpperCase());
						$("#lintasanku").html(": " + nama_lintasan.toUpperCase());
