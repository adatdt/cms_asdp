...
class M_gs extends MY_Model{
	...
	{
		return $this->db->query("
			select count(b.ship_name) as total_kendaraan, b.ship_name , a.name as tipe_name 
	,b.ship_id, b.boarding_code, a.id
	from app.t_mtr_vehicle_class a
	left join(
		select br.boarding_code, br.ship_id, bk.vehicle_class_id, sc.schedule_date, sp.name as ship_name 
		from app.t_trx_open_boarding br
