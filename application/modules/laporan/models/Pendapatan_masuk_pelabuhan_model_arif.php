...
/**
 * -----------------------
 * CLASS NAME : Pendapatan_masuk_pelabuhan_model
 * -----------------------
 *
...
class Pendapatan_masuk_pelabuhan_model extends MY_Model{
	...
	public function sub_total_vehicle($where=""){
		...
		return $this->db->query("SELECT count( distinct bv.id) as ticket_count, sum(bv.entry_fee) as total_amount
			...
			join app.t_trx_booking_vehicle bv on bv.booking_code = bok.booking_code and bv.status = 5 --and bv.vehicle_class_id is not null
			{$where}");
	}
