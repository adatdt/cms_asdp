...
class M_stc extends MY_Model{
	...
	function get_list_dock(){
		...
		if($port_id){
			...
			if($check_schedule){
				...
				foreach ($query as $row) {
					$data[$row->name] = $this->get_list_schedule($row->id);
				}
		...
		if($this->check_trx_schedule($mtr_schedule[0]['dock_id'],$param[0])){
			...
			switch ($type) {
				...
				case 3:
					...
					// $this->db->insert($t_open_boarding,array_merge($data_open_boarding,$default));
					// $this->db->insert($t_end_unload,array_merge($data_end_unload,$default));
				...
				case 4:
					...
					// $this->db->insert($t_close_boarding,array_merge($data_close_boarding,$default));
	...
	function gate_action($param){
		...
		$row = $this->db->query($schedule)->result_array()[0];
