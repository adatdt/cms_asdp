...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		$("#port").change(function() {
			$.ajax({
					method: "GET",
					url: "ticket_terpadu_tertagih_vm/getvm/" + $("#port").val(),
					type: "html"
				})
				.done(function(msg) {
					$("#vm").html(msg);
		...
		$("#printerkita").on("click", function(e) {
			...
			window.location = "<?php echo site_url('laporan/ticket_terpadu_tertagih_vm/get_pdf?port=') ?>" + port +
				"&datefrom=" + datefrom +
				"&dateto=" + dateto +
				"&petugas=" + petugas +
				"&vm=" + vm +
		...
		$("#excelkita").on("click", function(e) {
			...
			window.location = "<?php echo site_url('laporan/ticket_terpadu_tertagih_vm/get_excel?port=') ?>" + port +
				"&datefrom=" + datefrom +
				"&dateto=" + dateto +
				"&regu=" + regu +
				"&petugas=" + petugas +
		...
		$("#cari").on("click", function(e) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('laporan/ticket_terpadu_tertagih_vm') ?>",
				dataType: 'json',
				data: {
					port: $("#port").val(),
					datefrom: $("#datefrom").val(),
				...
				success: function(json) {
					...
					if (json.code == 200) {
						...
						html += "<tr><td colspan='4'> <b>TOTAL JUMLAH (VM)</b></td><td class='text-right'>" + formatIDR(jumlah_produksi) + "</td><td class='text-right'>" + formatIDR(jumlah_pendapatan) + "</td></tr>";
