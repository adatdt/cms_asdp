...
class Ticket_terpadu_tertagih extends MY_Controller
{
	...
	public function index()
	{
		...
		if ($this->input->is_ajax_request()) {
			...
			$tunai = $this->m_tertagih->list_data($port, $datefrom, $dateto, $petugas, $shift, $ship_class, 'tunai', $vm);
			$cashless = $this->m_tertagih->list_data($port, $datefrom, $dateto, $petugas, $shift, $ship_class, 'cashless', $vm);
			$online = $this->m_tertagih->list_data($port, $datefrom, $dateto, $petugas, $shift, $ship_class, 'online', $vm);
			$ifcs = $this->m_tertagih->list_data($port, $datefrom, $dateto, $petugas, $shift, $ship_class, 'ifcs', $vm);
			$ifcs_reedem = $this->m_tertagih->list_data($port, $datefrom, $dateto, $petugas, $shift, $ship_class, 'ifcs_reedem', $vm);
			$b2b = $this->m_tertagih->list_data($port, $datefrom, $dateto, $petugas, $shift, $ship_class, 'b2b', $vm, $merchant);
