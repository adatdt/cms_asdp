...
<page backtop="27mm" backbottom="8mm" backleft="7mm" backright="7mm">
	<page_header>
	<!-- Setting Header -->
		...
		<tr>
			...
			<th class="center bold" style="width: 15%">TARIF
				...
				<th class="center bold" style="width: 15%">PRODUKSI
					...
					foreach ($penumpang['data'] as $key_pnp => $pnp) {
						$produksi_penumpang += $pnp->ticket_count;
						$pendapatan_penumpang += $pnp->total_amount;
						?>
						<tr>
							<td></td>
							<td><?= $pnp->name?></td>
							<td class="right"><?= idr_currency($pnp->ifpro_fee) ?></td>
					...
					foreach ($kendaraan['data'] as $key_vhc => $vhc) {
						$produksi_kendaraan += $vhc->ticket_count;
						$pendapatan_kendaraan += $vhc->total_amount;
						?>
						<tr>
							<td></td>
							<td><?= $vhc->name?></td>
							<td class="right"><?= idr_currency($vhc->ifpro_fee) ?></td>
			...
			$filename = strtoupper("Pendapatan_ifpro" . $cabang . "_" . $this->input->get('datefrom') . "_" . $this->input->get('dateto'));
			try
			{
