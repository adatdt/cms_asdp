...
/*
  add token csrf enchance 17-07-2023
  by adat   
 */
...
class Gs extends MY_Controller{
    ...
    public function detail($boarding_code)
    {
        ...
        // enchance to db escape
        $code=$this->db->escape($this->enc->decode($boarding_code));
        ...
        $data['detail_passanger_vehicle']=$this->gs->list_detail_passanger_vehicle("where a.boarding_code=".$code."  and  e.service_id=2 and b.status=1 and c.status >= 5 ")->result();
        $data['passanger_vehicle_count']=$this->gs->list_detail_passanger_vehicle("where a.boarding_code=".$code."  and  e.service_id=2 and b.status=1 and c.status > 5")->num_rows();
    ...
    public function download_vehicle_passanger_05082021($encode)
    {
        ...
        $code=$this->enc->decode($encode);
        $data=$this->gs->list_detail_passanger_vehicle("where a.boarding_code='".$code."'  and  e.service_id=2 and b.status=1 and c.status<>'-5' ")->result();
        $excel = new Exceldownload();
        // Send Header
    ...
    public function download($encode)
    {
        ...
        $code=$this->db->escape($this->enc->decode($encode));
        $data=$this->gs->list_detail_passanger("where a.boarding_code = ".$code."  and  e.service_id=1 and b.status=1")->result();
        $excel = new Exceldownload();
        // Send Header
    ...
    function download_pdf()
    {
        ...
        $gender = $this->gs->get_sum_penumpang($code,"1,4");
        $anak = $this->gs->get_sum_penumpang($code,"2");
        $bayi = $this->gs->get_sum_penumpang($code,"3");
        ...
        $data['hitung_passanger_vehicle']=$this->gs->get_detail_passanger_vehicle2($code);
        $data['detail_passanger']=$this->gs->list_detail_passanger("where a.boarding_code='".$code."'  and  e.service_id=1 and b.status=1")->result();

        if ($n=='summary')
        {
            $data['detail_passanger_vehicle']=$this->gs->get_detail_passanger_vehicle($code);
