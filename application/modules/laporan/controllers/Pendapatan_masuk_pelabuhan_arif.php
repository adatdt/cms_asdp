...
class Pendapatan_masuk_pelabuhan extends MY_Controller {
	...
	public function detail($assignment_code){
        ...
        $data['detail_trip']   = $this->pendapatan_masuk->detail_trip(" where up.assignment_code = '$code' ")->row();
        $data['detail_passenger']   = $this->pendapatan_masuk->list_detail_passanger(" where ob.assignment_code = '$code' ")->result();
        $data['sub_total_passenger']   = $this->pendapatan_masuk->sub_total_passanger(" where ob.assignment_code = '$code' ")->row();
        $data['detail_vehicle']  = $this->pendapatan_masuk->list_detail_vehicle(" where ob.assignment_code = '$code' ")->result();
        $data['sub_total_vehicle']   = $this->pendapatan_masuk->sub_total_vehicle(" where ob.assignment_code = '$code' ")->row();
