...
error_reporting(0);
class Pendapatan_anchor extends MY_Controller {

	public function __construct(){
		parent::__construct();
		$this->load->model('pendapatan_anchor_model', 'model');
	...
	public function index(){
		...
		$data = array(
			...
			'title'    => 'PENDAPATAN ANCHOR',
			'content'  => $this->_module.'/index',
            'url'      => site_url('laporan/pendapatan_anchor/get_list_data'),
            'urlDownload' => site_url('laporan/pendapatan_anchor/download_pdf'),
            'urlDownload_excel' => site_url('laporan/pendapatan_anchor/download_excel'),
    ...
    function get_list_data(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $response = json_api(1,'List Report Anchor',$this->model->dataList());
        }   

        $this->log_activitytxt->createLog($this->_session->name, uri_string(), 'report anchor', json_encode($post), $response);
        echo $response;
        exit;
    }
	...
	function download_pdf() {
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $data['report_title'] = 'LAPORAN PRODUKSI DAN PENDAPATAN KAPAL RORO JASA SANDAR ENGKER';
    ...
    function download_excel(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $det  = $data['detail'];
            $excel_name = "LAPORAN PRODUKSI DAN PENDAPATAN KAPAL RORO JASA SANDAR ENGKER";
            ...
            $writer->writeSheetRow($excel_name, array("TANGGAL", "", $data['tanggal'],'','','','','',''), $style);
            ...
            $writer->markMergedCell($excel_name, $i,0,$i,1);
            $writer->markMergedCell($excel_name, $i,2,$i,8);
            ...
            $style['font-style'] = 'bold';
            $writer->writeSheetRow($excel_name, array('SUB TOTAL','','','','',$data['total_anchor'],$data['total_grt'],$data['sub_total'],''), $style);
