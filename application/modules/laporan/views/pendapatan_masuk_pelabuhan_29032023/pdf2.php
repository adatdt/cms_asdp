...
<page backtop="27mm" backbottom="8mm" backleft="7mm" backright="7mm">
	...
	<table class="tabel full-width" align="center">
		<tr>
				...
				<th class="center bold" style="width: 15%">PRODUKSI
					...
					foreach ($penumpang as $key_pnp => $pnp) {
						...
						<tr>
							<td></td>
							<td><?= $pnp->name?></td>
							<td class="right"><?= idr_currency($pnp->entry_fee) ?></td>
					...
					foreach ($kendaraan as $key_vhc => $vhc) {
						...
						<tr>
							<td></td>
							<td><?= $vhc->name?></td>
							<td class="right"><?= idr_currency($vhc->entry_fee) ?></td>
			...
			$filename = strtoupper("Pendapatan_pas_masuk_" . $cabang . "_" . $this->input->get('datefrom') . "_" . $this->input->get('dateto'));
			try
			{
