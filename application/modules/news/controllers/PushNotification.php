...
class PushNotification extends MY_Controller{
    ...
    public function action_add(){
        ...
        $this->form_validation->set_rules('startDate', 'Priode Awal', 'trim|required|callback_validate_date');
        $this->form_validation->set_rules('endDate', 'Priode Akhir', 'trim|required|callback_validate_date');
        $this->form_validation->set_rules('contentData', 'Konten', 'trim|required');

        $this->form_validation->set_message('required','%s harus diisi!');
        $this->form_validation->set_message('numeric','%s harus angka!');
        $this->form_validation->set_message('validate_date','%s Format Tanggal tidak sesuai !');
        ...
        {
            echo $res=json_api(0," Format file yang di perbolehkan JPG/JPEG atau PNG");     
        }
        ...
        else
        {
            ...
            if($sendData->status==1)
            {
                ...
                if(!empty($filename))
                {
                    ...
                    imagepng(imagecreatefromstring(file_get_contents($lokasi)), $path_grup.$nama_baru2.".jpeg");
    ...
    public function getDataUraian(){
        ...
        $getList->tokenHash=$this->security->get_csrf_hash();
        $getList->csrfName=$this->security->get_csrf_token_name();
    ...
    public function getDataTime(){
        ...
        $getTimeFrekuensi=$this->getTimeFrekuensi(); //date minutes
        if(!empty($getTimeFrekuensi)){
    ...
    // {
    //     $getParameter=$this->pushNotification->select_data("app.t_mtr_custom_param"," where status=1 and param_name='max_frekuensi' ")->row();
