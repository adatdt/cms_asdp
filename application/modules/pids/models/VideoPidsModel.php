...
class VideoPidsModel extends MY_Model{
  ...
  public function __construct() {
    ...
    $where = " WHERE vd.status not in (-5) ";

    if(!empty($port))
    {
        $where .=" and ( vd.port_id={$port}) ";
    }
    ...
    $sql  = "
                SELECT 
                p.name as port_name,
                vd.* from app.t_mtr_pids_video vd
                left join app.t_mtr_port p on vd.port_id=p.id
                {$where}
