...
class NewsPromo extends MY_Controller{
	public function __construct(){
		parent::__construct();

        logged_in();
        $this->load->model('newsPromoModel','newsPromo');
        $this->load->model('global_model');

        $this->_table    = '';
        $this->_username = $this->session->userdata('username');
        $this->_module   = 'news/newsPromo';
        $this->_pathFile  ="uploads/news/promo/";
        $this->_pathFileThumbnail  ="uploads/news_thumbnail/promo/";
	}

	public function index(){   
        checkUrlAccess(uri_string(),'view');
        if($this->input->is_ajax_request()){
            $rows = $this->newsPromo->getDataList();
    ...
    public function add(){
        ...
        $data['title'] = 'Tambah Berita Promo';
    ...
    public function action_add(){
        ...
        else
        {
            ...
            $sendData=$this->newsPromo->postData($urlApi,$data);
        ...
        $logUrl      = site_url().'news/newsPromo/action_add';
        $logMethod   = 'ADD';
    ...
    public function action_edit(){
        ...
        else
        {
         ...
         /* Fungsi Create Log */
        $createdBy   = $this->session->userdata('username');
        $logUrl      = site_url().'news/newsPromo/action_edit';
        $logMethod   = 'EDIT';
        $logParam    = json_encode($data);
        $logResponse = $res;
    ...
    public function action_delete($param){
        ...
        /* Fungsi Create Log */
        $createdBy   = $this->session->userdata('username');
        $logUrl      = site_url().'news/newsPromo/action_delete';
        $logMethod   = 'DELETE';
        $logParam    = json_encode($data);
        $logResponse = $res;
    ...
    public function action_change($param){
        ...
        /* Fungsi Create Log */
        $this->log_activitytxt->createLog($this->_username, uri_string().'news/newsPromo/change_status', 'change_status', json_encode($data), $response); 
    ...
    function convertTowebp_03102022($new_name, $new_name2, $count=0, $quality = 100)
    {
        $dir=$this->_pathFile;
        $mime_type = mime_content_type($dir.$new_name);
        if ($mime_type == 'image/jpeg') {
    ...
    function getParam()
    {
        $getParameter=$this->newsPromo->select_data("app.t_mtr_custom_param"," where status=1 and param_name='max_img_news' ")->row();
    ...
    {
        $dataParam = $this->newsPromo->select_data("app.t_mtr_custom_param"," where param_name='source_news_video'  and status=1 ")->row();
    ...
    {
        $getParameter=$this->newsPromo->select_data("app.t_mtr_custom_param"," where status=1 and param_name='range_minutes_frekuensi' ")->row();
