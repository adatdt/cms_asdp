...
class Ticket_refund_model extends CI_Model
{
    ...
    private function get_kendaraan($channel)
    {
        ...
        $where = "WHERE rf.is_approved IS TRUE AND bk.service_id = 2";
        ...
        if ($datefrom and $dateto) {
            $where .= " AND rf.approved_on::date between '{$datefrom}' and '{$dateto}'";
        }
        ...
        COALESCE (refund_fee, 0) AS refund_fee,
        COALESCE (charge_amount, 0) AS charge_amount,
        COALESCE (bank_transfer_fee, 0) AS bank_transfer_fee,
    ...
    LEFT JOIN (
        SELECT
            ...
            SUM (rf.refund_fee) AS refund_fee,
            SUM (rf.charge_amount) AS charge_amount,
            SUM (rf.bank_transfer_fee) AS bank_transfer_fee,
            SUM (rf.total_amount) AS total_amount
        FROM
            app.t_trx_refund rf
    ...
    private function get_penumpang($channel)
    {
        ...
        $where1 = "WHERE bp.service_id = 1 and rf.is_approved is true";

        $where2 = "where rf.is_approved is true and bk.service_id = 1";

        if ($datefrom and $dateto) {
            $where1 .= " AND  rf.approved_on::date between '{$datefrom}' and '{$dateto}'";
            $where2 .= " AND rf.approved_on::date between '{$datefrom}' and '{$dateto}'";
        }
        ...
        SUM(COALESCE ( fare, 0 )) AS fare,
        SUM(COALESCE ( produksi, 0 )) AS produksi,
        SUM(COALESCE ( adm_fee, 0 )) AS adm_fee,
        SUM(COALESCE ( refund_fee, 0 )) AS refund_fee,
        SUM(COALESCE ( charge_amount, 0 )) AS charge_amount,
        SUM(COALESCE (bank_transfer_fee, 0)) AS bank_transfer_fee,
    ...
    FROM
        ...
        LEFT JOIN (
            select	
                    ...
                    sum(fee.refund_fee) as refund_fee,
                    sum(fee.charge_amount) as charge_amount,
                    sum(fee.adm_fee) as adm_fee2,
                    sum(CASE
                        WHEN fee.rank = 1 THEN rf.bank_transfer_fee
                        ELSE null
                    END) as bank_transfer_fee,
                    sum(CASE
                        WHEN fee.rank = 1 THEN rf.total_amount
                        ELSE null
                    END) as total_amount
                FROM
                    app.t_trx_refund rf
                ...
                left join (SELECT *
                                FROM (
                                    ...
                                    JOIN app.t_trx_refund rf on rf.booking_code = bp.booking_code 
                                    JOIN app.t_mtr_ship_class sc ON sc.id = bk.ship_class 
                                    {$where1}
                                    ) t
