...
if ( ! function_exists("generate_menu") ) {
    ...
    function generate_menu($arr, $menu_order) {
        ...
        if ($menu_order == 0) {
            $html = "\n<ul class='page-sidebar-menu  page-header-fixed page-sidebar-menu-light ' data-keep-expanded='false' data-auto-scroll='false' data-slide-speed='200'>
...
if (! function_exists('generate_button') ) {
	function generate_button($slug, $action, $button) {
		...
		// FROM
		// 	core.t_mtr_user tmu
		// JOIN core.t_mtr_user_group tmg ON tmu.user_group_id = tmg.id
		// JOIN core.t_mtr_privilege tmp ON tmg.id = tmp.group_id
		// JOIN core.t_mtr_privilege_detail tmpd ON tmp.id = tmpd.privilege_id
		// JOIN core.t_mtr_menu tmm ON tmpd.menu_id = tmm.id
		// WHERE
		// 	tmu.id = '{$user_id}'
		// AND
		// 	tmm.slug = '{$slug}'
        ...
        JOIN core.t_mtr_menu_action ee ON ee.id = dd.action_id AND ee.status = 1 AND LOWER(ee.action_name) = '{$action}'
...
if (!function_exists('generate_button_new')) {
    function generate_button_new($slug, $action, $url) {
        $CI      =& get_instance();
        $user_id = $CI->session->userdata('id');
        $button  = '';
        ...
        if($access){
            if(strtolower($action) == 'add'){
                $button = '<button onclick="showModal(\''.$url.'\')" class="btn btn-sm btn-warning" title="Tambah"><i class="fa fa-plus"></i> Tambah</button> ';
            }
            ...
            elseif(strtolower($action) == 'delete'){
                $button = '<button class="btn btn-sm btn-danger" onclick="confirmationAction(\'Apakah Anda yakin menghapus data ini ?\', \''.$url.'\')" title="Hapus"> <i class="fa fa-trash-o"></i> </button>';
            }
            ...
            //     $button = '<a href="'.$url.'" class="btn btn-sm btn-primary"><i class="fa fa-search-plus" title="Detail"></i></a>';
            // }
...
if(!function_exists('checkAction')){
    function checkAction($menu_id){
        ...
        $sql = "SELECT ee.action_name FROM core.t_mtr_privilege_web aa
            JOIN core.t_mtr_user_group bb ON aa.user_group_id = bb.id AND bb.status = 1 AND bb.id = $group_id
            JOIN core.t_mtr_menu_web cc ON aa.menu_id = cc.id AND cc.status = 1 AND cc.id = $menu_id
            JOIN core.t_mtr_menu_detail_web dd ON aa.menu_detail_id = dd.id AND dd.status = 1
            JOIN core.t_mtr_menu_action ee ON ee.id = dd.action_id AND ee.status = 1
            WHERE aa.status = 1 ORDER BY cc.order ASC";
...
if(!function_exists('listMenuChild')){
    function listMenuChild($parent = 0, $data){
        $CI   =& get_instance();
        $uri  = uri_string();

        if (isset($data[$parent])) {
            if($parent == 0){
                $html = '<ul class="page-sidebar-menu  page-header-fixed page-sidebar-menu-light" data-keep-expanded="false" data-auto-scroll="false" data-slide-speed="200">';
            ...
            foreach ($data[$parent] as $v) {
                ...
                if($child){
                    ...
                    if (substr(trim($v->icon), 1, 3) == 'svg') {
                        $html .= '<span class="svg-icon">' . $v->icon . '</span>';
                    } else {
                    ...
                    $html .= '<li class="nav-item"><a href="'.$link.'">';
                    if (substr(trim($v->icon), 1, 3) == 'svg') {
                        $html .= '<span class="svg-icon">' . $v->icon . '</span>';
                    } else {
                        $html .= '<i class="fa fa-'.$v->icon.'"></i>';
                    }
                    $html .= '<span class="title">'.$v->name.'</span></a></li>';
                }
...
if(!function_exists('checkUrlAccess')){
    function checkUrlAccess($current_url,$action){
        ...
        $sql = "SELECT cc.id, cc.parent_id, cc.name, icon, slug, cc.order FROM
            core.t_mtr_privilege_web aa
        JOIN core.t_mtr_user_group bb ON bb.id = aa.user_group_id AND bb.status = 1 AND bb.id = $group_id
        JOIN core.t_mtr_menu_web cc ON cc.id = aa.menu_id AND cc.status = 1 AND slug = '$current_url'
        JOIN core.t_mtr_menu_detail_web dd ON dd.id = aa.menu_detail_id AND dd.status = 1
        JOIN core.t_mtr_menu_action ee ON ee.id = dd.action_id AND ee.status = 1 AND LOWER(ee.action_name) = '{$action}'
        WHERE aa.status = 1 ORDER BY cc.order ASC";
...
if(!function_exists('checkBtnAccessDetailDashboard')){
    function checkBtnAccessDetailDashboard($current_url,$action){
        // $CI     =& get_instance();
        // $session= $CI->session->userdata('app_session');
        // $menu   = getMenu($action);
        // $link   = array();
