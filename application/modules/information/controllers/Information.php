...
class Information extends MY_Controller {
    ...
    {
		 $this->check_access('information', 'view');
    	if ($this->input->server('REQUEST_METHOD') == 'POST')
    	{
    		$rows = $this->information_model->informationList();
	...
	function approve()
	{
		...
		if (!empty($booking_vehicle_id))
		{
			...
			foreach ($booking_vehicle_id as $booking_vehicle_id)
			{
				...
				$datavehicle2=$this->sail_model->getVehicle($departure, $depart_date, $ship_id)->result();
			...
			$this->db->insert_batch("app.t_trx_sail_vehicle",$insert2);
		}
