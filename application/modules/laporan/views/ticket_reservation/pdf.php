...
<page backtop="30mm" backbottom="10mm" backleft="8mm" backright="8mm">
	...
	<table class="tabel full-width" style="width: 100% !important; min-width: 100% !important">
		...
		foreach ($data as $row) {
			...
			foreach ($row['data'] as $rowA) {
				...
				foreach ($rowA['data'] as $k => $v) {
					...
					echo '
						<tr>
							...
							<td class="right">' . $v->produksi . '</td>
							<td class="right">' . $v->pendapatan . '</td>
						</tr>';
					$noRow++;
				}
				...
				<tr class="bold">
					...
					<td class="right">' . $cProduksi1 . '</td>
					<td class="right">' . $cPendapatan1 . '</td>
				</tr>';
				$noRow++;
			...
			<tr class="bold">
				...
				<td class="right">' . $cProduksi2 . '</td>
				<td class="right">' . $cPendapatan2 . '</td>
			</tr>
			';
			$noRow++;
		...
		<tr class="bold">
			...
			<td class="right">' . $cProduksi3 . '</td>
			<td class="right">' . $cPendapatan3 . '</td>
		</tr>';
		?>
	</table>
...
$filename = strtoupper("Laporan_tiket_reservasi_" . $portname . "_" . format_date($datefrom) . "_" . format_date($dateto) . "_" . $ship_classku);
try {
	// setting paper
