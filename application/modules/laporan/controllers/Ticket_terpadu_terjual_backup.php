...
class Ticket_terpadu_terjual extends MY_Controller
{
	...
	public function index()
	{
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($payment_type === "all") {
				$tunai = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'tunai', $vm);
				$cashless = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'cashless', $vm);
				$online = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'online', $vm);
				$ifcs = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'ifcs', $vm);
				$ifcs_reedem = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'ifcs_reedem', $vm);
				$b2b = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'b2b', $vm, $merchant);
			...
			} elseif ($payment_type === "ifcs_reedem") {
				$lintasanku = "-";
				$ifcs_reedem = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'ifcs_reedem', $vm);
	...
	function get_pdf()
	{
		...
		if ($payment_type === "all") {
			...
			$ifcs = $this->m_terjual->list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, 'ifcs_reedem', $vm);
	...
	function get_excel()
	{
		...
		if ($payment_type === "all") {
			...
			$produksi_ifcs_reedem = 0;
			$pendapatan_ifcs_reedem = 0;

			foreach ($ifcs_reedem as $key => $value) {
				$produksi_ifcs_reedem += $value->produksi;
				$pendapatan_ifcs_reedem += $value->pendapatan;
			...
			if ($ifcs_reedem) {
				foreach ($data_reedem as $row) {
					$writer->writeSheetRow($sheet1, $row, $styles2);
				}
			}

			$writer->writeSheetRow($sheet1, array("Sub Total", "", "", $produksi_ifcs_reedem, $pendapatan_ifcs_reedem), $style_sub);
			$writer->writeSheetRow($sheet1, array(""));
			...
			$writer->writeSheetRow($sheet1, array("LINTASAN", $lintasanku, "", "TANGGAL", format_date($datefrom) . " - " . format_date($dateto)));
			$writer->writeSheetRow($sheet1, array("SHIFT", $shift, "", "VENDING MACHINE", $vmkita));
			$writer->writeSheetRow($sheet1, array("STATUS", $keterangan_report, "", "MERCHANT", $merchantku));
			$writer->writeSheetRow($sheet1, array(""));
			$writer->writeSheetRow($sheet1, array("TIPE PENJUALAN : B2B"));
			$writer->writeSheetRow($sheet1, $judul_tabel, $styles1);
