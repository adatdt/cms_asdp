...
error_reporting(0);
class Pendapatan_asuransi extends MY_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('pendapatan_asuransi_model', 'model');
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'title'          => 'Pendapatan Asuransi Iuran Wajib (Jasa Raharja)',
    ...
    public function old_detail($shift_date, $ship_class_id, $shift_id)
    {
        ...
        $data['header'] = $header;
        $data['report_title'] = "FORMULIR LAPORAN PENDAPATAN ASURANSI IURAN WAJIB PER-SHIFT";
    ...
    public function detail()
    {
        ...
        if (!$penumpang && !$kendaraan) {
            ...
            $input = array(
                ...
                'penumpang' => $penumpang,
                'kendaraan' => $kendaraan,
                'report_title' => "FORMULIR LAPORAN PENDAPATAN ASURANSI IURAN WAJIB PER-SHIFT",
                'cabang' => $cabang,
                'pelabuhan' => $pelabuhan,
                'lintasan' => $lintasan,
                'shift' => $shift_name,
    ...
    function get_pdf()
    {
        ...
        $data['penumpang'] = $penumpang;
        $data['kendaraan'] = $kendaraan;
        $data['persen_jr'] = $this->model->persen_jr();
    ...
    function get_excel()
    {
        ...
        $filename = strtoupper("Asuransi_jr_" . $pelabuhan . "_" . $datefrom . "_" . $dateto . ".xlsx");
        ...
        foreach ($penumpang as $key => $value) {
            ...
            $pendapatan_penumpang += $value->insurance_fee;
        ...
        foreach ($kendaraan as $key => $value) {
            ...
            $pendapatan_kendaraan += $value->insurance_fee;
    ...
    function download_excel($shift_date, $ship_class_id, $shift_id)
    {
        ...
        $this->load->library('XLSExcel');
        $writer = new XLSXWriter();
        $filename = strtoupper("Asuransi_jr_" . $shift_date . ".xlsx");
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="' . $filename . '"');
        header('Cache-Control: max-age=0');
