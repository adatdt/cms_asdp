...
class EntranceFee extends MY_Controller{
	public function __construct(){
		parent::__construct();

        logged_in();
        $this->load->model('EntranceFeeModel','fare');
        $this->load->model('global_model');
        $this->load->library('log_activitytxt');

        $this->_table    = 'app.t_mtr_entrance_fee_passanger';
	...
	public function index(){   
        ...
        if($this->input->is_ajax_request()){
            ...
            if($this->enc->decode($this->input->post('passangerType'))){
                $this->form_validation->set_rules('passangerType', 'Golongan', 'trim|callback_validate_decode_param', array('validate_decode_param' => 'Invalid Golongan '));
            }
        ...
        $getPassangerType = $this->fare->select_data("app.t_mtr_passanger_type"," where status=1 order by name asc ")->result();
        ...
        foreach ($getPassangerType as $key => $value) {
        ...
        $data = array(
            ...
            'title'    => 'Master Tarif Entrance Penumpang',
            'content'  => 'fare/entranceFee/index',
            'port' => $dataPort,
    ...
    public function add(){
        ...
        $data['title'] = 'Tambah Tarif Entrance Penumpang';
        $data['port'] = $dataPort;
    ...
    public function action_add()
    {
        ...
        $this->form_validation->set_rules('passangerType', 'Golongan Penumpang ', 'trim|required|callback_validate_decode_param', array('validate_decode_param' => 'Invalid Golongan Penumpang '));
        ...
        else 
        {
            ...
            $passangerType = trim($this->enc->decode($this->input->post('passangerType')));
            $entranceFee = trim($this->input->post('entranceFee'));
            
            $data=array(
                        'port_id'=>$port,
                        'passanger_type_id'=>$passangerType,
            ...
            $check=$this->fare->select_data($this->_table," where port_id={$port} and passanger_type_id={$passangerType} and status not in (-5) ");
        ...
        $logUrl      = site_url().'fare/entrance/action_add';
        $logMethod   = 'ADD';
    ...
    public function edit($id)
    {
        ...
        $dataPassangerType[""]="Pilih";
        $dataPassangerTypeSelected="";
        foreach ($getPassangerType as $key => $value) {
            $idkey=$this->enc->encode($value->id);
            if($value->id == $getDetail->passanger_type_id)
            {
                $dataPassangerTypeSelected = $idkey;
            }
            $dataPassangerType[$idkey] = strtoupper($value->name);
        }
        ...
        $data['passangerType'] = $dataPassangerType;
        $data['passangerTypeSelected'] = $dataPassangerTypeSelected;
    ...
    public function action_edit()
    {
        ...
        else 
        {
            ...
            // ceck data jika username sudah ada
            $check=$this->fare->select_data($this->_table," where port_id={$port} and passanger_type_id={$passangerType} and status not in (-5) and id<>{$id} ");
