...
<body>
    ...
    <div width="50%"  style="padding-bottom: 10px;" >
        ...
        <div style="padding-bottom:5px;" class="text-header2"><b>Rincian Pesanan</b></div>        
        <div class="lines"></div>
    </div>
    ...
    <div width="50%"  style="padding-bottom: 10px;" >
        ...
        <table width="100%" class="blueTable" class="text-footer" border=0>
            <tbody>
                ...
                <?php
                    $subtotal = $detailRincianVehicle[0]->fare ; 
                    foreach($detailRincianVehicle as $keyRincian => $valueRincian) 
                { ?>
                <tr style="text-align: center" class="text-small">                
                    <td align="left">1. <?= @$masterVehicle[$valueRincian->vehicle_class] ?> (x1)</td> 
                ...
                <tr >
                    <td colspan=2 align="right" class="tdrincian">
                        <b>Biaya Administarasi</b><br>
                        <div class="text-placeholder"><?= $vehicle[0]->getNamePayment; ?> </div>                   
                    </td>
                    <td align="right" class="tdrincian"><b>Rp<?=number_format($vehicle[0]->amount_payment - $vehicle[0]->amount_invoice,0,',','.')?></b></td> 
                <tr>                
                <tr >                 
                    <td colspan=2 align="right" class="tdrincian">
                    ...
                    <td class="tdrincian" align="right" style="background-color:#f2f2f2; border-top:1px solid white; border-bottom:1px solid white;">
                        <b>Rp<?=number_format($vehicle[0]->amount_payment,0,',','.')?></b></td> 
                <tr>
                <?php if(strtoupper($vehicle[0]->payment_type) == "CASH" ) { 
                    
                    $cashData = $vehicle[0]->amount_invoice;
                    if(!empty($vehicle[0]->total_cash))
                    {
                        $cashData = $vehicle[0]->total_cash;
                    }
                ?>
                <tr >                 
