			...
			<div class="portlet-body">
				<div class="row">
					...
					<div id="master_tabel" style="display: none">
						...
						<?= generate_button('laporan/pendapatan_ifpro', 'download_pdf', '<button id="printerkita" class="btn btn-sm btn-default"><i class="fa fa-file-pdf-o" style="color: #ea5460"></i> PDF</button>') ?>
						<?= generate_button('laporan/pendapatan_ifpro', 'download_excel', '<button id="excelkita" class="btn btn-sm btn-default"><i class="fa fa-file-excel-o" style="color: #ea5460"></i> EXCEL</button>') ?>
						...
						<table class="tabel full-width" align="center"> 
							<tr>
								...
								<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
									FORMULIR LAPORAN PENDAPATAN<br>TERMINAL EKSEKUTIF IFPRO PER-SHIFT</span>
								</td>
...
<script type="text/javascript">
	jQuery(document).ready(function () {
		...
		$("#printerkita").on("click",function(e){
			...
			window.location = "<?php echo site_url('laporan/pendapatan_ifpro/get_pdf?port=') ?>"+port+
			"&datefrom=" +datefrom+
			"&dateto=" +dateto+
			"&shift="+shift+
		...
		$("#excelkita").on("click",function(e){
			...
			window.location = "<?php echo site_url('laporan/pendapatan_ifpro/get_excel?port=') ?>"+port+
			"&datefrom=" +datefrom+
			"&dateto=" +dateto+
			"&shift="+shift+
			"&ship_class="+ship_class+
		...
		$("#cari").on("click",function(e){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('laporan/pendapatan_ifpro/detail') ?>",
				dataType: 'json',
				data: { 
					port: $("#port").val(),
					datefrom: $("#datefrom").val(),
			...
			success: function(json) {
				...
				if (json.code == 200) {
					...
					$.each(json.penumpang.data, function(i, item) {
						$("#master_tabel").show();
						...
						html += "<td class='text-right'>" + formatIDR(item.ifpro_fee) + "</td>";
					...
					$.each(json.kendaraan.data, function(i, item) {
						$("#master_tabel").show();
