...
class Bank extends MY_Controller{
    ...
    public function action_add()
    {
        ...
        $abbr=trim($this->input->post('abbr', true));
        ...
        $check=$this->bank->select_data($this->_table," where upper(bank_abbr)=upper(".$this->db->escape($abbr).") and status not in (-5) ");
    ...
    public function action_edit()
    {
        ...
        else
        {
         ...
         /* Fungsi Create Log */
        $createdBy   = $this->session->userdata('username');
        $logUrl      = site_url().'master_data/bank/action_edit';
        $logMethod   = 'EDIT';
        $logParam    = json_encode($data);
        $logResponse = $res;
