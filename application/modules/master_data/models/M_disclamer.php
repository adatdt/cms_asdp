...
class M_disclamer extends MY_Model{
    ...
    public function dataList(){
		...
		{
			$where .="	and name ilike '%".$search['value']."%' ESCAPE '!' 
								or info ilike '%".$search['value']."%' ESCAPE '!' ";
		}

		$sql 		   = "
							select * from app.t_mtr_info
							{$where}
						 ";
		...
		foreach ($rows_data as $row) {
			...
			if($row->status == 1){
				...
				$row->actions  .= generate_button($this->_module, 'edit', '<button onclick="showModal_new(\''.$edit_url.'\')" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></button> ');
