...
class M_stc extends MY_Model
{
	...
	function get_list_dock()
	{
		...
		if ($port_id) {
			...
			if ($check_schedule) {
				$sql = "SELECT id, name, terminal_code
				FROM app.t_mtr_dock tmd
				LEFT JOIN app.t_mtr_device_terminal tmdt ON tmd.id = tmdt.dock_id AND tmdt.terminal_type = 8 AND tmdt.status = 1
				WHERE tmd.status = 1 AND tmd.port_id = {$this->enc->decode($post['port'])}
				ORDER BY name ASC";
	...
	function get_list_schedule($dock)
	{
		...
		foreach ($query as $key => $row) {
			...
			$row->url = site_url("transaction/stc/edit/" . $row->param);
			...
			switch ($type) {
				...
				case 5:
					...
					if ($row && $row['plot_code']) {
						$listTerminalCode = $this->listManlessGate($row['port_id'], $row['dock_id']);
	...
	function listManlessGate_10112020($port_id, $dock_id = "")
	{
		$dock = "";
		if ($dock_id != "") {
	...
	function listManlessGate($port_id, $dock_id = "")
	{
