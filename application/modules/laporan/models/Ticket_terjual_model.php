...
class Ticket_terjual_model extends CI_Model
{
	...
	public function list_data($port, $datefrom, $dateto, $regu, $petugas, $shift, $ship_class, $type, $vm = "", $ticket_type = "", $merchant = "")
	{
		...
		if ($ticket_type != "") {
			if ($ticket_type == 1) { // ticket normal
				$where_ticket_passanger = " AND BP.ticket_type != 3 ";
				$where_ticket_vehicle = " AND BV.ticket_type != 3 ";
			...
			$sql = "SELECT
						...
						FROM
							...
							LEFT JOIN 	(SELECT
										...
										FROM
											app.t_trx_booking_passanger BP
											JOIN app.t_trx_booking BO ON BO.booking_code = BP.booking_code AND BO.channel IN ( 'web', 'mobile','web_cs' ) $where_ship_class
					...
					SELECT
						...
						FROM
							...
							LEFT JOIN 	(SELECT
										...
										FROM
											app.t_trx_booking_vehicle BV
											JOIN app.t_trx_booking BO ON BO.booking_code = BV.booking_code AND BO.channel IN ( 'web', 'mobile' ,'web_cs') $where_ship_class
											JOIN app.t_trx_payment PY ON PY.trans_number = BO.trans_number
