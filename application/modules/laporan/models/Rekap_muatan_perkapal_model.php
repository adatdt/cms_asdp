...
class Rekap_muatan_perkapal_model extends MY_Model
{
	...
	public function get_kapal_company($user_id)
	{
		return $this->dbView->query("SELECT
								...
								FROM
									...
									JOIN app.t_mtr_ship tship ON tship.ship_company_id = tuship.company_id and tship.status !='-5'
								WHERE
	...
	public function dataList()
	{
		...
		if ($group_id == 11 or $group_id == 39 ) {
			$where .= " AND ttob.ship_id IN (SELECT
										tship.id
									FROM
										app.t_mtr_user_ship tuship
										JOIN core.t_mtr_user usere ON usere.id = tuship.user_id
		...
		if(!empty($ticketType)) // tipe tiket 3 adalah tiket manual
		{
			if($ticketType==3)
			{
				...
				$where .= " and left(sc.schedule_code, 1 )='M' ";
			}
			else
			{
				$where .= " and left(sc.schedule_code, 1 ) !='M' ";
		...
		$sql = "SELECT ttob.schedule_code, 
					...
					ttob.port_id, tmp.name as port_name,
					tmsc.name as ship_class_name, 
					sc.open_boarding_date,
				...
				join app.t_trx_schedule sc on ttob.schedule_code=sc.schedule_code
		...
		foreach ($rows_data as $row) {
			...
			switch (substr($row->schedule_code,0,1)) {
				case 'M':
					$row->ticket_type="MANUAL" ;
					break;
	...
	public function detail_trip($where = "")
	{
		$sql = "select 
				...
				ship_company.name as company_name, 
				port.name as port_name,
				origin.name||'-'||destination.name as trip,
