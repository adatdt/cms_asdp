...
class Produksi_kapal_roro_gs_model extends MY_Model{
	...
	public function get_all_data_13012021($datefrom, $dateto, $port, $shift)
	{
		$where_port = "";
		$where_port_sub = "";
		// $where_ship_class = "";
	...
	public function get_all_data_02092021($datefrom, $dateto, $port, $shift)
	{
		$where_port = "";
		$where_port_sub = "";
		// $where_ship_class = "";
		...
		if ($rows_data) {
			...
			$sql2 = "SELECT
					...
					WHERE
						...
						ttbp.adm_fee
						,ttob.schedule_code
					) trx_passenger on passanger_type.id = trx_passenger.passanger_type_id where passanger_type.id in (1, 2, 3)";
			$result = $this->dbView->query($sql2);
			$penumpangs = $result->result();
		...
		foreach ($rows_data as $row) {
			...
			foreach ($penumpangs as $key_penumangs => $value_penumpangs) {
				...
				if ($value_penumpangs->schedule_code == $row->schedule_code) {
					...
					else if($value_penumpangs->passanger_id == 3)
					{
						$row->bayi += $value_penumpangs->ticket_count;	
					}
				}
				$row->totalP = $row->dewasa + $row->anak + $row->bayi;
			}
	...
	public function get_all_data($datefrom, $dateto, $port, $shift, $ticketType="")
	{
		$where_port = "";
		$where_port_sub = "";
		// $where_ship_class = "";
		...
		//  if the users are ship operator or ship owner  
		...
		if ($group_id == 11 || $group_id == 39 ) {
			...
			$dataShip = $this->db->query($qryUser);
			if($dataShip->num_rows()>0)
			{
				$shipCompanyId =array();
				foreach ($dataShip->result() as $key => $value) {
					$shipCompanyId[]="'".$value->company_id."'";
				}

				$whereIdCompany .= " AND SC.id IN (".implode(", ",$shipCompanyId).")";
			}
		...
		if ($rows_data) {
			...
			$whereTicketTypePassanger="";
			$whereTicketTypeVehicle="";

			if(!empty($ticketType))
			{
				if($ticketType==3) // ticket tipe 3 adalah tiket manual 
				{

					$whereTicketTypePassanger= " and ttbp.ticket_type=3 ";
					$whereTicketTypeVehicle= " and ttbv.ticket_type=3 ";
				}
				else
				{	
					$whereTicketPassanger= " and ttbp.ticket_type != 3 ";
					$whereTicketTypeVehicle= " and ttbv.ticket_type != 3 ";
				}
			}
			...
			$sql2 = "SELECT
					...
					FROM
						app.t_trx_booking ttb
						JOIN app.t_trx_booking_passanger ttbp ON ttbp.booking_code = ttb.booking_code $whereTicketTypePassanger
						AND ttbp.status = 5 
						AND ttb.service_id = 1
						JOIN app.t_trx_boarding_passanger ttop ON ttop.ticket_number = ttbp.ticket_number
						JOIN app.t_trx_open_boarding ttob ON ttob.boarding_code = ttop.boarding_code 
					...
					WHERE
						...
						ttbp.adm_fee
						,ttob.schedule_code
					) trx_passenger on passanger_type.id = trx_passenger.passanger_type_id where passanger_type.id in (1,2,3,4)";
			$result = $this->dbView->query($sql2);
			$penumpangs = $result->result();
			...
			$sql3 = "SELECT
					...
					FROM
						app.t_trx_booking ttb
						JOIN app.t_trx_booking_vehicle ttbv ON ttbv.booking_code = ttb.booking_code $whereTicketTypeVehicle
						AND ttbv.status = 5 
						AND ttb.service_id = 2
						JOIN app.t_trx_boarding_vehicle ttop ON ttop.ticket_number = ttbv.ticket_number
						JOIN app.t_trx_open_boarding ttob ON ttob.boarding_code = ttop.boarding_code 
		...
		foreach ($rows_data as $row) {
			...
			foreach ($penumpangs as $key_penumangs => $value_penumpangs) {
				...
				if ($value_penumpangs->schedule_code == $row->schedule_code) {
					...
					else if($value_penumpangs->passanger_id == 4)
					{
						$row->lansia += $value_penumpangs->ticket_count;	
					}					
				}
				$row->totalP = $row->dewasa + $row->anak + $row->bayi + $lansia;
			}
