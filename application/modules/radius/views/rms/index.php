                        ...
                        <div class="row">
                            <div class="col-sm-12 form-inline pad-top">
                                ...
                                <div class="input-group select2-bootstrap-prepend pad-top">
                                    ...
                                    <input type="text" class="form-control date input-small" id="reservation_date" readonly placeholder="YYYY-MM-DD">
                                </div>
...
<?php include "modal_user_detail.php" ?>
<?php include "modal_user_ifcs_detail.php" ?>
    ...
    var table= {
        loadData: function() {
            const getTable = $('#dataTables').DataTable({
                "ajax": {
                    "url": "<?php echo site_url('radius/rms') ?>",
                    "type": "POST",
                    "data": function(d) {
                        d.port = document.getElementById('port').value;
                        d.dateFrom = document.getElementById('dateFrom').value;
                        d.dateTo = document.getElementById('dateTo').value;
                        d.reservation_date = document.getElementById('reservation_date').value;
            ...
            $('#dataTables tbody').on('click', 'td.details-control', function () {
                ...
                if ( row.child.isShown() ) {
                    ...
                    myData.detailUserWebExp(row.data())  
                    ...
                    myData.detailUserWebLimit(row.data())  
                    $(`#add_detail_web_${row.data().id}`).on("click", function(){
                        $(`#detail_code_rms`).val(row.data().rms_code_enc)     
                        $(`#code_rms`).val(row.data().rms_code)     
                        $("#getEmail").tagsinput('removeAll');
                    ...
                    myData.detailUserIfcsExp(row.data())  
                    $(`#add_detail_ifcs_${row.data().id}`).on("click", function(){
                        $(`#detail_code_rms_ifcs`).val(row.data().rms_code_enc)     
                        $(`#code_rms_ifcs`).val(row.data().rms_code)    
                        $("#getEmailIfcs").tagsinput('removeAll'); 
                    })

                    myData.detailUserIfcsLimit(row.data())  
                    $(`#add_detail_ifcs_${row.data().id}`).on("click", function(){
                        $(`#detail_code_rms_ifcs`).val(row.data().rms_code_enc)     
                        $(`#code_rms_ifcs`).val(row.data().rms_code)    
                        $("#getEmailIfcs").tagsinput('removeAll'); 
