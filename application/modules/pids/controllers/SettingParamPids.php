...
class SettingParamPids extends MY_Controller{
    ...
    public function action_add()
    {
        ...
        if(strtoupper($typeInputValueParam)==strtoupper("text") or strtoupper($typeInputValueParam)==strtoupper("html") )
        {
            $value_param=trim($this->input->post('value_param'));
            $this->form_validation->set_rules('value_param', 'Value Parameter ', 'required');
        }
    ...
    public function edit($id)
    {
        ...
        else if($detail->param_type_value=='html')
        {

            $paramTypeValueHtml .='<div class="form-group">
                                    <label>Value Param<span class="wajib">*</span></label>
                                    <textarea class="wysihtml5 form-control" name="value_param" id="value_param" placeholder="Info" required  rows="20">'.$detail->param_value.'</textarea>
                                </div>';
            $paramTypeValue="";
        ...
        $data['paramTypeValueHtml']=$paramTypeValueHtml;
        $data['id']=$id;
        $data['port'] = $portData;
        $data['portDataSelected'] = $portDataSelected;
    ...
    public function action_change($param)
    {
        ...
        {
            $this->db->trans_commit();
            $port = $this->param->select_data($this->_table, " where id=".$d[0])->row()->port_id;
            $returnData = array("portId"=>$port);            
            if ($d[1]==1)
            {
    ...
    public function action_delete($id)
    {
        ...
        {
            $this->db->trans_commit();
            $port = $this->param->select_data($this->_table, " where id=".$id)->row()->port_id;
            $returnData = array("portId"=>$port);            
