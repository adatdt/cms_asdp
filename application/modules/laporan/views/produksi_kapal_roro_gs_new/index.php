...
<script type="text/javascript">
	jQuery(document).ready(function () {
		...
		$("#printerkita").on("click",function(e){
			...
			window.location = "<?php echo site_url('laporan/produksi_kapal_roro_gs_new/get_pdf?port=') ?>"+port+
			"&datefrom=" +datefrom+
			"&dateto=" +dateto+
			"&shift="+shift+
		...
		$("#excelkita").on("click",function(e){
			...
			window.location = "<?php echo site_url('laporan/produksi_kapal_roro_gs_new/get_excel?port=') ?>"+port+
			"&datefrom=" +datefrom+
			"&dateto=" +dateto+
			"&shift="+shift+
			// "&ship_class="+ship_class
		...
		$("#cari").on("click",function(e){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('laporan/produksi_kapal_roro_gs_new/detail') ?>",
				dataType: 'json',
				data: { 
					port: $("#port").val(),
					datefrom: $("#datefrom").val(),
				...
				success: function(json) {
					...
					if (json.code == 200) {
						...
						html += "<td class='text-center' colspan='2'> PENUMPANG </td>";
