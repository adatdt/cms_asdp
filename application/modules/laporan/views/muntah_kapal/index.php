			...
			<div class="portlet-body">
				...
				<div id="master_tabel" style="display: none">
					...
					<div class="form-inline">
						...
						<?= generate_button('laporan/muntah_kapal', 'download_pdf', '<button id="printerkita" class="btn btn-sm btn-default"><i class="fa fa-file-pdf-o" style="color: #ea5460"></i> PDF</button>') ?>
						<?= generate_button('laporan/muntah_kapal', 'download_excel', '<button id="excelkita" class="btn btn-sm btn-default"><i class="fa fa-file-excel-o" style="color: #ea5460"></i> EXCEL</button>') ?>
					...
					<table class="tabel full-width" align="center">
						<tr>
							...
							<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
								LAPORAN REKAPITULASI MUNTAH KAPAL</span>
							</td>
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		$("#port").change(function() {
			$.ajax({
					method: "GET",
					url: "muntah_kapal/get_regu/" + $("#port").val(),
					type: "html"
				})
				.done(function(msg) {
					$("#regu").html(msg);
		...
		$("#printerkita").on("click", function(e) {
			...
			window.location = "<?php echo site_url('laporan/muntah_kapal/get_pdf?port=') ?>" + port +
				"&datefrom=" + datefrom +
				"&dateto=" + dateto +
		...
		$("#excelkita").on("click", function(e) {
			...
			window.location = "<?php echo site_url('laporan/muntah_kapal/get_excel?port=') ?>" + port +
				"&datefrom=" + datefrom +
				"&dateto=" + dateto +
				"&ship_class=" + ship_class +
				"&shift=" + shift +
		...
		$("#cari").on("click", function(e) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('laporan/muntah_kapal') ?>",
				dataType: 'json',
				data: {
					port: $("#port").val(),
					datefrom: $("#datefrom").val(),
