				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="row">
							...
							<!-- filtering untuk tiket manual tapi belum naik -->
							<!-- <div class="col-md-3" >
								<div class="form-group">
									<div class="input-group">
						...
						<div id="master_tabel" style="display: none">
							...
							<?= generate_button('laporan/pendapatan_masuk_pelabuhan', 'download_pdf', '<button id="printerkita" class="btn btn-sm btn-default"><i class="fa fa-file-pdf-o" style="color: #ea5460"></i> PDF</button>') ?>
							<?= generate_button('laporan/pendapatan_masuk_pelabuhan', 'download_excel', '<button id="excelkita" class="btn btn-sm btn-default"><i class="fa fa-file-excel-o" style="color: #ea5460"></i> EXCEL</button>') ?>
							

							<!-- <button id="printerkita" class="btn btn-sm btn-default"><i class="fa fa-file-pdf-o" style="color: #ea5460"></i> PDF</button> -->

							<!-- <button id="excelkita" class="btn btn-sm btn-default"><i class="fa fa-file-excel-o" style="color: #ea5460"></i> EXCEL</button> -->
							...
							<table class="tabel full-width" align="center">
								<tr>
									...
									<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
										FORMULIR LAPORAN PENDAPATAN<br>PAS MASUK PELABUHAN PER-SHIFT</span>
									</td>
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {
			...
			$("#printerkita").on("click", function(e) {
				...
				window.location = "<?php echo site_url('laporan/pendapatan_masuk_pelabuhan/get_pdf?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&shift=" + shift +
			...
			$("#excelkita").on("click", function(e) {
				...
				window.location = "<?php echo site_url('laporan/pendapatan_masuk_pelabuhan/get_excel?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&shift=" + shift +
					"&pelabuhan=" + pelabuhan +
			...
			$("#cari").on("click", function(e) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('laporan/pendapatan_masuk_pelabuhan/detail') ?>",
					dataType: 'json',
					data: {
						port: $("#port").val(),
						datefrom: $("#datefrom").val(),
					...
					success: function(json) {
						...
						if (json.code == 200) {
							...
							html += "<td class='text-center'> TARIF (PAS) </td>";
							...
							$.each(json.penumpang, function(i, item) {
								$("#master_tabel").show();
								total_produksi_penumpang += Number(item.ticket_count);
								total_pendapatan_penumpang += Number(item.total_amount);

								html += "<tr>";
								html += "<td class='text-center'>" + angka++ + "</td>";
								html += "<td class='text-left'>" + item.name + "</td>";
								html += "<td class='text-right'>" + formatIDR(item.entry_fee) + "</td>";
								html += "<td class='text-right'>" + formatIDR(item.ticket_count) + "</td>";
								html += "<td class='text-right'>" + formatIDR(item.total_amount) + "</td>";
							...
							$.each(json.kendaraan, function(i, item) {
								$("#master_tabel").show();
								total_produksi_kendaraan += Number(item.ticket_count);
								total_pendapatan_kendaraan += Number(item.total_amount);
							...
							var pelabuhan = ": " + $('#port').find(":selected").text();
