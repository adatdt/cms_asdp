								...
								<div class="col-md-3" style="padding-left: 5px;">
									<div class="form-group">
										<div class="input-group">
											...
											<select id="ship_class" class="form-control select2" dir="">
												<option value="">Semua</option>
				...
				<div id="master_tabel" style="display: none">
					...
					<table class="tabel full-width" align="center"> 
						<tr>
							...
							<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
								FORMULIR LAPORAN PENDAPATAN DERMAGA IV INFINITY PER-SHIFT</span>
							</td>
					...
					<table class="table table-333 table-bordered full-width" align="center">
						...
						<tr>
							...
							<td style="border-left: none !important;">: <b><span id="status_approve"></span></span></b></td>
						</tr>
						</table>
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function () {
			...
			$("#port").change(function() {
				$.ajax({
					method: "GET",
					url: "dermaga_infinity/get_regu/"+$("#port").val(),
					type: "html"
				})
				.done(function( msg ) {
					$("#regu").html(msg);
			...
			$("#printerkita").on("click",function(e){
				...
				window.location = "<?php echo site_url('laporan/dermaga_infinity/get_pdf?port=') ?>"+port+
				"&datefrom=" +datefrom+
				"&dateto=" +dateto+
				"&regu=" +regu+
				"&petugas="+petugas+
			...
			$("#excelkita").on("click",function(e){
				...
				window.location = "<?php echo site_url('laporan/dermaga_infinity/get_excel?port=') ?>"+port+
				"&datefrom=" +datefrom+
				"&dateto=" +dateto+
				"&regu=" +regu+
				"&petugas="+petugas+
			...
			$("#cari").on("click",function(e){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('laporan/dermaga_infinity/detail') ?>",
					dataType: 'json',
					data: { 
						port: $("#port").val(),
						datefrom: $("#datefrom").val(),
						...
						cabang: $('#port').find(":selected").text(),
						shiftku: $('#shift').find(":selected").text(),
						reguku: $('#regu').find(":selected").text(),
						petugasku: $('#petugas').find(":selected").text(),
					},
					...
					success: function(json) {
						...
						if (json.code == 200) {
							...
							html += "<td class='text-center'> TARIF (90% * Dmg)</td>";
							...
							$.each(json.penumpang, function(i, item) {
								produksi_penumpang += Number(item.produksi);
								pendapatan_penumpang += Number(item.pendapatan);
							...
							html += "<tr><td colspan='3'> <b>Sub Total</b></td><td class='text-right'>" +formatIDR(produksi_penumpang)+ "</td><td class='text-right'>" +formatIDR(pendapatan_penumpang)+ "</td></tr>";
							...
							$.each(json.kendaraan, function(i, item) {
								produksi_kendaraan += Number(item.produksi);
								pendapatan_kendaraan += Number(item.pendapatan);
							...
							total_produksi = produksi_penumpang + produksi_kendaraan;
							total_pendatapan = pendapatan_penumpang + pendapatan_kendaraan;

							html += "<tr><td colspan='3'> <b>Sub Total</b></td><td class='text-right'>" +formatIDR(pendapatan_penumpang)+ "</td><td class='text-right'>" +formatIDR(pendapatan_penumpang)+ "</td></tr>";
							
							html += "<tr><td colspan='3'> <b>TOTAL JUMLAH (Penumpang + Kendaraan)</b></td><td class='text-right'>" +formatIDR(total_produksi)+ "</td><td class='text-right'>" +formatIDR(total_pendatapan)+ "</td></tr>";
