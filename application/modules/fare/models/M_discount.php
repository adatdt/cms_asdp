...
class M_discount extends MY_Model{
    ...
    public function dataList(){
		...
		{
			$where .="and (a.schema_code ilike '%".$iLike."%' ESCAPE '!'
							or a.discount_code ilike '%".$iLike."%' ESCAPE '!'
							or a.description ilike '%".$iLike."%' ESCAPE '!'
							or b.description ilike '%".$iLike."%' ESCAPE '!'
							)";
		}


		$sql 		   = "
							SELECT d.name as port_name, 
							c.discount_code, b.slug, b.description as schema_name2, a.* from app.t_mtr_discount a
		...
		foreach ($rows_data as $row) {
			...
			$edit_url 	 = site_url($this->_module."/edit/{$this->enc->encode($row->discount_code)}");
     		...
     		if ($row->end_date > $dateNow && $row->status == 1) {
				$row->status   = success_label('Aktif');
				$row->actions  = generate_button_new($this->_module, 'edit', $edit_url);
				$row->actions .= generate_button($this->_module, 'delete', '<button class="btn btn-sm btn-danger" onclick="confirmationAction(\'Apakah Anda yakin akan menonaktifkan data ini ?\', \''.$nonaktif.'\')" title="Nonaktifkan"> <i class="fa fa-ban"></i> </button> ');
			}else if ($row->end_date > $dateNow && $row->status == -1){
				...
				$row->status   = failed_label('Tidak Aktif');
				$row->actions .= generate_button($this->_module, 'delete', '');
			}
	...
	public function get_detail_fare_passanger($discount_code, $ship_class)
	{
	...
	public function get_detail_fare_vehicle($discount_code, $ship_class)
	{
	...
	{
		return $this->select_data("app.t_mtr_ship_class"," where status not in (-5) order by id ")->result();
	}
	...
	public function download_excel(){
		...
		$sql 		   = "
							SELECT d.name as port_name, c.discount_code, b.slug, b.description as schema_name1, a.* from app.t_mtr_discount a
							left join app.t_mtr_discount_schema b on a.schema_code=b.schema_code 
							left join 
							(select distinct discount_code, port_id from app.t_mtr_discount_detail_port) 
							c on a.discount_code=c.discount_code
