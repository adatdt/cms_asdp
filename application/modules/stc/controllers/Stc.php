...
class Stc extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('master_data/m_setting_param', 'param');
        ...
        // set notif_gate_trigger 
        $this->_notif_gate_trigger = $this->param->select_data("app.t_mtr_custom_param", "where param_name = 'notif_gate_trigger'")->row();
        $this->_socket_protocol = $this->config->item('socket_protocol');
        $this->_socket_transport = $this->config->item('socket_transport');
        $this->_socket_url = $this->config->item('socket_url');
        $this->_dashboard_socket_key = $this->config->item('dashboard_socket_key');
    }
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'port_name' => $port_name,
            'socket_transport' => $this->_socket_transport,
            'socket_protocol' => $this->_socket_protocol,
            'socket_url' => $this->_socket_url,
            'dashboard_socket_key' => $this->_dashboard_socket_key,
            'notif_gate_trigger' => $this->_notif_gate_trigger->param_value,
            'problem' => array(
                ...
                array('id' => 'list-broken', 'problem' => $this->enc->encode(4), 'list' => 'List Broken', 'title' => 'Broken'),
            ),
            'url_problem' => site_url($this->_module . '/problem'),
            'list_gate' => $this->stc->listGate($this->session->userdata('port_id'))
        );

        $this->load->view('stc/default', $data);
    }
    ...
    public function edit($param)
    {
        ...
        if ($param) {
            ...
            $data = array(
                'title' => strtoupper('Aksi Kapal'),
                'ship'  => $this->stc->get_list_ship($this->session->userdata('port_id'), $this->stc->get_id_schecule($params[0])),
                // 'ship_class'  => $this->stc->get_ship_class_id($params[3])->ship_class,
                'ship_class'  => empty($params[3])?"":$this->stc->get_ship_class_id($params[3])->ship_class,    // handling jika tidak ada nama kapalnya       
                'ship_class_by_dock'  => $this->stc->get_ship_class_id_by_dock($params[11])->ship_class,
    ...
    public function action_edit()
    {
        ...
        //ngambil param buat notif max_open_boarding_ptc
        $max = $this->db->select('
                                param_name,
                                param_value
                                ')
            ...
            if (isset($post['call'])) {
                if ($post['call'] == '') {
                    ...
                    if ($query) {
                        ...
                        $res = json_api(1, 'Update Data Berhasil', array('post' => $post, 'portId' => $this->session->userdata('port_id'), 'shipId' => $ship));
                    } else {
                ...
                if ($type == 3 and !$this->stc->get_shift($param[12])) {
                    ...
                    if ($query == 1) {
                        ...
                        $res = json_api(0, 'Dermaga yang Mulai Pelayanan tidak boleh lebih dari '. $max[0]['param_value'] .' Dermaga');
                    } elseif ($query) {                       
                        $res = json_encode($this->db->error());
                    }
                }
    ...
    {
        $data = $this->stc->get_list_manless($port, $dock);
        $obj = array();
