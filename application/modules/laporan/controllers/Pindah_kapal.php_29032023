...
class Pindah_kapal extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('pindah_kapal_model', 'm_pindah');
	...
	public function index()
	{
		...
		if ($this->input->is_ajax_request()) {
			...
			$cek_sc   	= $this->m_pindah->getClassBySession();
			...
			$lintasan 		    = $this->m_pindah->get_lintasan($port, $datefrom, $dateto, $ship_class, $shift);
			$ship_origin        = $this->m_pindah->get_ship_origin($port, $datefrom, $dateto, $ship_class, $shift);
			$ship_destination   = $this->m_pindah->get_ship_destination($port, $datefrom, $dateto, $ship_class, $shift);
			$penumpang 		    = $this->m_pindah->list_data($port, $datefrom, $dateto, $ship_class, $shift, $regu, "penumpang");
			$kendaraan 		    = $this->m_pindah->list_data($port, $datefrom, $dateto, $ship_class, $shift, $regu, "kendaraan");
			$lintasanku 	    = "-";
			$ship_originku      = "-";
			$ship_destinationku = "-";
		...
		$data = array(
			...
			'title'			 => 'Laporan Pindah Kapal',
			'content'        => 'laporan/pindah_kapal/index',
			'port' 			 => $this->m_pindah->getport(),
	...
	{
		$shift_class = $this->m_pindah->getClassBySession('option');
	...
	function get_regu($port_id = "")
	{
		...
		if (!$port_id) {
			...
			$data = $this->m_pindah->get_team($port_id);
			$option = '<option value="" selected>Semua</option>';
	...
	function get_pdf()
	{
		...
		$this->load->view('laporan/pindah_kapal/pdf', $data);
	}

	function get_excel()
	{
		$excel_name = "Laporan_pindah_kapal";
		...
		$filename = strtoupper("Laporan_pindah_kapal_" . $pelabuhan . "_" . trim($datefrom, "-") . "-" . trim($datefrom, "-") . "_" . $ship_class . ".xlsx");
		...
		$writer->writeSheetRow($sheet1, array("LAPORAN REKAPITULASI PINDAH KAPAL"), $styles1);
