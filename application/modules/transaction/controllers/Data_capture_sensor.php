...
class Data_capture_sensor extends MY_Controller{
	public function __construct(){
		parent::__construct();

        logged_in();
        $this->load->model('M_data_capture_sensor','sensor');
	...
	public function index(){   
        ...
        if($this->input->is_ajax_request()){
            $rows = $this->sensor->dataList();
            echo json_encode($rows);
            exit;
        }

        $get_identity=$this->sensor->get_identity_app();
        ...
        if($get_identity==0)
        {
            ...
            {
                $port=$this->sensor->select_data("app.t_mtr_port","where id=".$this->session->userdata('port_id')."");
                $getRoute=$this->sensor->getRoute($this->session->userdata('port_id'))->result();
            }
            else
            {
                $dataPort[""]="Semua";
                $dataRoute[""]="Pilih";
                $port=$this->sensor->select_data("app.t_mtr_port","where status not in (-5) order by name asc");
        ...
        {
            $port=$this->sensor->select_data("app.t_mtr_port","where id=".$get_identity."");
            $getRoute=$this->sensor->getRoute($get_identity)->result();
        }


        $dataShipClass[""]="Pilih";
        $getShipClass=$this->sensor->select_data("app.t_mtr_ship_class", " where status<>'-5' order by name asc")->result();
        ...
        $data = array(
            ...
            'title'    => 'Data Capture Sensor',
            'content'  => 'data_capture_sensor/index',
            'service'  => $this->sensor->select_data("app.t_mtr_service","where status=1 order by name asc")->result(),
            'port'=>$dataPort,
            'route'=>$dataRoute,
            'shipClass'=>$dataShipClass,
            'destination'=>$port=$this->sensor->select_data("app.t_mtr_port","where status not in (-5) order by name asc")->result(),
            'team'=>$this->sensor->select_data("core.t_mtr_team","where status=1 order by team_name asc")->result(),
    ...
    public function getRoute()
    {
        ...
        if(!empty($port))
        {
            ...
            $route=$this->sensor->getRoute($port)->result();
    ...
    public function download_excel()
    {
        ...
        $data = $this->sensor->download()->result();
    ...
    public function download_pdf()
    {
        ...
        if (!empty($port))
        {
            $getDataPort=$this->sensor->select_data("app.t_mtr_port", " where id='{$port}' ")->row();

            $portName=strtoupper($getDataPort->name);
        }

        $data['data'] = $this->sensor->download()->result();
        $data['port'] = $portName;
