<?php
error_reporting(0);
class Muntah_kapal_v2 extends MY_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('muntah_kapal_model_v2', 'model_muntah');
    ...
    public function index()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $rows = $this->model_muntah->dataList();
        ...
        if ($group_id == 11) {
            $ship = $this->model_muntah->get_kapal_company($this->session->userdata('id'));
        }

        $data = array(
            'home'          => 'Beranda',
            'url_home'      => site_url('home'),
            'title'         => 'Laporan Muntah Kapal V2',
            'content'       => 'muntah_kapal_v2/index',
    ...
    public function detail($switch_ship_code)
    {
        ...
        $data['title']              = 'LAPORAN MUNTAH KAPAL';
        $lintasan                   = $this->model_muntah->get_lintasan($switch_ship_code);
        $detail_trip                = $this->model_muntah->detail_trip($switch_ship_code);
        $data['lintasan']           = $lintasan->port_origin . " - " . $lintasan->port_destination;
        $data['detail_trip']        = $detail_trip;
        $data['detail_passenger']   = $this->model_muntah->list_detail_passanger($switch_ship_code);
        $data['detail_vehicle']     = $this->model_muntah->list_detail_vehicle($switch_ship_code);
    ...
    function download_excel($switch_ship_code)
    {
        ...
        $detail_trip = $this->model_muntah->detail_trip($switch_ship_code);
        $detail_passenger = $this->model_muntah->list_detail_passanger($switch_ship_code);
        $detail_vehicle = $this->model_muntah->list_detail_vehicle($switch_ship_code);

        $this->load->library('XLSExcel');
        $writer = new XLSXWriter();
        $filename = strtoupper("muntah_kapal_" . $detail_trip->ship_name . "_" . $detail_trip->shift_date . ".xlsx");
        ...
        $writer->writeSheetRow($sheet1, array("LAPORAN MUNTAH KAPAL"), $style_header);
