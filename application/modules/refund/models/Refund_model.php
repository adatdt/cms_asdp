...
class Refund_model extends MY_Model{
	...
	{
		return $this->dbView->query("SELECT c.phone_number, c.extra_fee, c.amount as tarif, c.total_amount, c.customer_name,c.email,b.channel, a.* from app.t_trx_refund a
	...
	{
		$rows_data =  $this->dbView->query("SELECT a.*, c.description from app.t_log_refund_transfer a
	...
	{
		$rows_data =  $this->dbView->query("SELECT a.*, c.description, b.is_approved, b.approved_on from app.t_log_refund_transfer a
									left join app.t_trx_refund b on a.refund_code=b.refund_code  and b.status<>'-5'
									left join app.t_mtr_status c on a.transfer_status = c.status and c.tbl_name = 't_trx_refund'
									where a.status<>'-5' and a.booking_code='{$bookingCode}' --and a.transfer_status not in (4,5,6,1)
									order by a.created_on desc 
	...
	{
		return $this->dbView->query("SELECT bukti_transfer from app.t_trx_refund_bukti_transfer a
	...
	{
		return $this->dbView->query("SELECT bukti_nodin from app.t_trx_refund_bukti_nodin a
									where a.status<>'-5' and a.refund_code='{$refund_code}'
							  ");
	}
