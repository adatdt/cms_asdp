...
class PassangerRescheduleTicketModel extends MY_Model
{
	...
	public function dataList()
	{
		...
		$where = " WHERE rs.status = 1 AND bk.service_id = 1  and (date(bk.depart_date) between '" . $dateFrom . "' and '" . $dateTo . "' ) ";
	...
	public function qry($where = "", $order = "")
	{
		return $data = 
			"SELECT
				...
				( CASE WHEN inv2.status = 2 THEN 'SUDAH DIBAYAR' WHEN inv2.status IN ( 0, 1 ) THEN 'BELUM DIBAYAR' ELSE'' END ) AS desc_status_reschedule 
			FROM
