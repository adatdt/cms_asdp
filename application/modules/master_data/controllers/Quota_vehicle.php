...
class Quota_vehicle extends MY_Controller{
	public function __construct(){
		parent::__construct();

        logged_in();
        $this->load->model('m_quota_vehicle','quota');
        $this->load->model('global_model');
        $this->load->library('log_activitytxt');

        $this->_table    = 'app.t_mtr_booking_quota_vehicle';
	...
	public function index(){   
        ...
        $data = array(
            ...
            'title'    => 'Quota Kendaraan',
            'content'  => 'quota_vehicle/index',
            ...
            'vehicle_type' =>$this->quota->select_data("app.t_mtr_vehicle_type"," where status not in (-5) order by name asc ")->result(),
        );
    ...
    public function add(){
        ...
        $data['title'] = 'Tambah Quota Kendaraan';
        ...
        $data['vehicle_type'] =$this->quota->select_data("app.t_mtr_vehicle_type"," where status not in (-5) order by name asc ")->result();
    ...
    public function action_add()
    {
        ...
        $this->form_validation->set_rules('vehicle_type', 'Tipe Kendaraan', 'required');
        ...
        $check=$this->quota->select_data($this->_table," where port_id=$port and ship_class=$ship_class and vehicle_type=$vehicle_type and status not in (-5) ");
        ...
        $logUrl      = site_url().'master_data/quota_vehicle/action_add';
        $logMethod   = 'ADD';
    ...
    public function edit($id)
    {
        ...
        $data['title'] = 'Edit Quota Kendaraan';
        $data['detail']=$this->quota->select_data($this->_table,"where id=$id_decode")->row();
        $data['port'] = $this->quota->select_data("app.t_mtr_port"," where status=1 order by name asc")->result();
        $data['ship_class'] = $this->quota->select_data("app.t_mtr_ship_class"," where status=1 order by name asc")->result();
        $data['vehicle_type'] = $this->quota->select_data("app.t_mtr_vehicle_type"," where status=1 order by name asc")->result();
    ...
    public function action_edit()
    {
        ...
        // ceck data jika port dan class kapal sudah ada 
        $check=$this->quota->select_data($this->_table," where port_id=$port and ship_class=$ship_class and vehicle_type=$vehicle_type and  status not in (-5) and id!=$id");
        ...
        else
        {
         ...
         /* Fungsi Create Log */
        $createdBy   = $this->session->userdata('username');
        $logUrl      = site_url().'master_data/quota_vehicle/action_edit';
        $logMethod   = 'EDIT';
        $logParam    = json_encode($data);
        $logResponse = $res;
