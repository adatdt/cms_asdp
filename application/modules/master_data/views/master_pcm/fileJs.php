    ...
    class MyData{
        ...
        getTime(date, token="")
        {
            $.ajax({
                ...
                data        : "datePick="+date+"&<?= $this->security->get_csrf_token_name() ?>="+token,
                type        : 'POST',
                dataType    : 'json',
                beforeSend: function(){
                    unBlockUiId('box')
                ...
                success: function(x)
                {
                    let dataTime = x.dataTime
                    var html="<option value=''>Pilih</option>";
                    for(var i=0; i < dataTime.length; i++)
                    {
                        html +=`<option value='${dataTime[i].idData}' ${dataTime[i].statusData} >${dataTime[i].valData}</option>`
                    }
                    $(`[name ='<?= $this->security->get_csrf_token_name() ?>']`).val(x.tockenHash);
                    $("#time").html(html)
                },
                complete: function(){
                    $('#box').unblock(); 
