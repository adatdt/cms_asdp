			...
			<div class="portlet-body">
				<div class="row">
					<div class="col-md-12">
						...
						<?= generate_button('laporan/pendapatan_asuransi_jrp', 'download_pdf', '<button id="printerkita" class="btn btn-sm btn-default"><i class="fa fa-file-pdf-o" style="color: #ea5460"></i> PDF</button>') ?>
						<?= generate_button('laporan/pendapatan_asuransi_jrp', 'download_excel', '<button id="excelkita" class="btn btn-sm btn-default"><i class="fa fa-file-excel-o" style="color: #ea5460"></i> EXCEL</button>') ?>
							...
							<table class="tabel full-width" align="center">
								<tr>
									...
									<td rowspan="4" class="center bold" style="width: 60%;font-size: 14pt; line-height: 1.5">
										FORMULIR LAPORAN PENDAPATAN ASURANSI<br>TG. JAWAB PENGANGKUT PER-SHIFT</span>
									</td>
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {
			...
			$("#printerkita").on("click", function(e) {
				...
				window.location = "<?php echo site_url('laporan/pendapatan_asuransi_jrp/get_pdf?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&shift=" + shift +
			...
			$("#excelkita").on("click", function(e) {
				...
				window.location = "<?php echo site_url('laporan/pendapatan_asuransi_jrp/get_excel?port=') ?>" + port +
					"&datefrom=" + datefrom +
					"&dateto=" + dateto +
					"&shift=" + shift +
					"&ship_class=" + ship_class +
			...
			$("#cari").on("click", function(e) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('laporan/pendapatan_asuransi_jrp/detail') ?>",
					dataType: 'json',
					data: {
						port: $("#port").val(),
						datefrom: $("#datefrom").val(),
					...
					success: function(json) {
						...
						if (json.code == 200) {
							...
							html += "<td class='text-center'> TARIF (JRP) </td>";
							...
							$.each(json.penumpang, function(i, item) {
								...
								total_pendapatan_penumpang += Number(item.responsibility_fee);
								...
								if (item.responsibility_fee != null) {
									pendapatanku = formatIDR(item.responsibility_fee)
								}
							...
							$.each(json.kendaraan, function(i, item) {
								...
								total_pendapatan_kendaraan += Number(item.responsibility_fee);
